...
def Ghexin(X, y, max_height, min_epochs, min_purity, epsilon_w, epsilon_n, min_size):
	...
	n_nodes = 0
	root = GhexinNode('Node_' + str(n_nodes), parent=None, X=X, y=y, sample_indeces=[], w=centroid_X, T=np.Inf)
...
#		print("Vertical growth - height = %d" %(k))
		...
		for i in range(0, n_leaves):
			...
			n = GhexinNode('Node_' + str(n_nodes), parent=parent, X=[], y=[], sample_indeces=[], w=parent.w+noise)
...
#							parent.draw_graph()
							...
							new_node = GhexinNode('Node_' + str(n_nodes), parent=parent, X=[], y=[], sample_indeces=[], w=sample)
...
nn = Ghexin(X, y, max_height=3, min_epochs=3, min_purity=0.6, epsilon_w=0.2, epsilon_n=0.01, min_size=5)
