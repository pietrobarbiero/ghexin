function [PSNR_ghexin, n_leaves_ghexin, ghexin_avg_silhouette, ghexin_dbindex, ET_ghexin, ...
          ...
          PSNR_ghng, n_leaves_ghng, ghng_avg_silhouette, ghng_dbindex, ET_ghng] = test_models_density(seed)

%% GHEXIN

% clc; dbstop if error; close all; %clear all; 
rng(seed)
...
f = plotCluster(Samples, nodesFirstLevel, 0);
hold off
f2 = plotCluster(Samples, leaves, 0);
hold off
...
MaxNeurons = 5; % Maximum number of neurons in each graph
Tau = 0.3;
MaxLevels = 2;
...
rmpath(genpath('../../src/matlab/ghng'));
